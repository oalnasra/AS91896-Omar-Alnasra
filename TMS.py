...
def new_tasks():
    ...
    name_to_id = {member["Name"]: member_id for member_id, member \
    in team_members.items()}
    selected_name = easygui.choicebox("Select Task Assignee:", \
    ...
    status = easygui.choicebox("Choose Task Status:", \
...
def update_task():
    ...
    if current_assignee in team_members and task_id in \
        team_members[current_assignee]["Tasks_Assigned"]:
...
def search_tasks(selected_choice):
    if selected_choice == "Task":
        ...
        selected_title = easygui.choicebox("Select a Task:", \
        choices=task_titles)
...
def search_members(selected_choice):
    if selected_choice == "Team Member":
        member_names = [member["Name"] for member in \
        team_members.values()]
        selected_member = easygui.choicebox("Select a Team Member:", \
        ...
        for member_id, member_info in team_members.items():
            if member_info["Name"] == selected_member:
                ...
                task_assignments = [f"- {tasks[task_id]['Title']} \
({task_id})" for task_id in assigned_tasks \
                if task_id in tasks]
                info_summary = "\n".join(task_assignments)
                member_info = f"Name: {member_info['Name']}\nID: \
{member_id}\nEmail: \
{member_info['Email']}\n\nAssigned Tasks:\n{info_summary}"
                easygui.msgbox(member_info, \
                title="Team Member Details")


def search_members_or_tasks():
    selected_choice = easygui.buttonbox("Search:", choices=["Task", \
    "Team Member", "Cancel"])
